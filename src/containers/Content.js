...
export default function ContentContainer({ details }) { 
    ...
    const keywordItems = details.keywords.keywords.map((keyword, idx) => (
        <Keywords.Item key={idx}>{keyword.name}</Keywords.Item>
    ))
    ...
    const recommendationsSliderItems = recommendations.map(item => (
        <Slider.Item to={`/movie/${item.id}`} key={`${item.order}-${item.id}`} imgHeight="140px" ratio="1.775">
            <Slider.Item.Image
