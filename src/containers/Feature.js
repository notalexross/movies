...
import { VideoPlayerContainer } from '.'
import {
    formatDateGB,
    formatTime,
    getYear,
...
export default function FeatureContainer({ details }) { 
    ...
    let providers, justWatchLink
    if (details.providers && details.providers.GB && details.providers.GB.flatrate) {
        providers = details.providers.GB.flatrate.slice(0, maxProviders).sort((a, b) => (
    ...
    let releaseDateRegion, certificationGB
        ...
        <Feature backgroundSrc={details.backdrop_path && `${backdropUrl}${details.backdrop_path}`} >
            ...
            <Feature.Content>
                ...
                <Feature.Actions>
                    ...
                    {videos &&
                        <Feature.Action>
                            <Feature.Trailer onClick={handleOpenTrailer}>
                                Play Trailer
                            </Feature.Trailer>
                        </Feature.Action>
                    }
                ...
                <Feature.People>
                    ...
                    {Array(maxCrew - crew.length + 1).fill().map((_, idx) => (
                        <div key={idx}></div>
                    ))}
                </Feature.People>
            </Feature.Content>
